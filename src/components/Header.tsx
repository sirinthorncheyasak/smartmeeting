...
import { User, LogOut, CheckCircle, Calendar, Shield, Cpu } from "lucide-react";
...
export default function Header({
  ...
  // Keep a digital clock ticking to look exceptionally modern
  useEffect(() => {
    const tick = () => {
      const now = new Date();
  ...
  return (
    <header className="glass-panel sticky top-4 z-30 px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4 rounded-[2rem] mx-4 md:mx-6 mt-4 max-w-7xl shadow-xl border border-white/55">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-500 font-semibold tracking-wider uppercase">Room Reservation System</p>
        </div>
      </div>
      ...
      {user && (
        <div className="flex items-center gap-4 flex-wrap justify-center">
          ...
          <div className="bg-white/40 border border-white/70 px-3 py-1.5 rounded-full flex items-center gap-1.5 font-mono text-xs text-slate-600 font-semibold shadow-inner">
            <span className="w-1.5 h-1.5 rounded-full bg-indigo-600 animate-pulse"></span>
            {timeStr} UTC
          </div>

          {/* Developer Role Simulation Toggle (Crucial convenience for evaluation) */}
          <div className="bg-amber-500/10 border border-amber-300/30 px-3 py-1 rounded-full flex items-center gap-2">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-left leading-tight hidden sm:block">
              <p className="text-xs font-bold text-slate-900">{user.displayName || "BU Member"}</p>
